...
async def test_vad_turn_streams_raw_pcm_audio(
    ...
    """A VAD-committed turn emits text and non-empty raw PCM before completion."""
    ...
    assert "response.audio.delta" in types, types
    assert "response.text.done" in types, types
    assert "response.audio.done" in types, types

    response_done_index = types.index("response.done")
    assert types.index("response.text.done") < response_done_index
    assert types.index("response.audio.done") < response_done_index
    assert response_done_index < types.index(
    ...
    assert any(sample != 0 for (sample,) in struct.iter_unpack("<h", pcm))

    response_done = events[response_done_index]["response"]
    assert response_done["status"] == "completed"
    content_types = {
        content["type"] for content in response_done["output"][0]["content"]
    }
    assert content_types == {"text", "audio"}
