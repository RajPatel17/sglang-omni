...
class MossTranscribeDiarizeForConditionalGeneration(nn.Module):
    ...
    def init_encoder_cache(self, max_bytes: int) -> None:
        """Enable the CPU-backed encoder-output cache; ``max_bytes`` <= 0 disables it.

        Stored on CPU so long-audio outputs don't contend with the KV cache in GPU
        memory; a hit costs only a host->device copy, far cheaper than a re-encode.
        """
        self._encoder_cache = (
            StageOutputCache(
    ...
    def get_audio_feature(
        ...
        # Only the single-item dispatch is cached; multi-item falls through.
